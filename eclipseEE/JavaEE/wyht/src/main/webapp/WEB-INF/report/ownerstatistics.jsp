...
var disnamedata=[];
	<c:forEach var="item" items="${list }">
		disnamedata.push({name:"${item.dis.dname}",value:"${item.hlid}",disid:"${item.dis.disid}"},);
	</c:forEach>
window.setTimeout(function(){
	ownerChart.hideLoading();//隐藏加载
ownerChart.setOption(option={
	title : {
        text: '总计\t:\t'+disnamedata.length,
        x:'center'
   	},
    tooltip : {
        trigger: 'item',
...
//echarts 绑定点击事件
ownerChart.on("click", eventClick);
});
//监听事件
function eventClick(param) {
    if (typeof param.seriesIndex != 'undefined') {
		if (param.type == 'click') {
			window.location.href='/wyht/report/ownerReport/'+param.data.disid;
		}	          
	}
}
