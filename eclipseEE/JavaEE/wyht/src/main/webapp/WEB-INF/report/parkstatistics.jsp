...
var disnamedata=[];
	<c:forEach var="item" items="${list }">
		disnamedata.push({name:"${item.dis.dname}",value:"${item.tollcount}",disid:"${item.dis.disid}"});
	</c:forEach>
...
//echarts 绑定点击事件
parkChart.on("click", eventClick);
});
//监听事件
function eventClick(param) {
    if (typeof param.seriesIndex != 'undefined') {
		if (param.type == 'click') {
			window.location.href='/wyht/report/parkReport/'+param.data.disid;
		}	          
	}
}
	...
	<div class="container col-sm-11" style="margin: 80px 0 0 90px;">
		<div class="card">
			...
			<div class="card-body card-padding">
				<form method="post" action="/wyht/report/parkReport/"
