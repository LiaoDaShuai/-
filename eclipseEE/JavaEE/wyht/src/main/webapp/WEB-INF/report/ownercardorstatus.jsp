...
$(function(){
var ownerChart = echarts.init(document.getElementById('owners-chart'),'macarons');
ownerChart.showLoading();//加载
var dname="";
var disnamedata=[{'name':'待审核',value:0},{'name':'住户',value:0},{'name':'亲属',value:0},
	{'name':'租户',value:0},{'name':'其他',value:0},];
	<c:forEach var="item" items="${list }">
		dname="${item.dis.dname}";
		if("${item.status}"==0){
			disnamedata[0].value="${item.hlid}";
		}else if("${item.status}"==2){
			if("${item.cardid}"==1){
				disnamedata[1].value="${item.hlid}";
			}else if("${item.cardid}"==2){
				disnamedata[2].value="${item.hlid}";
			}else if("${item.cardid}"==3){
				disnamedata[3].value="${item.hlid}";
			}else if("${item.cardid}"==4){
				disnamedata[4].value="${item.hlid}";
			}
		}
	</c:forEach>
window.setTimeout(function(){
	ownerChart.hideLoading();//隐藏加载
	...
	<div class="container col-sm-11" style="margin: 80px 0 0 90px;">
		<div class="card">
			...
			<div class="card-body card-padding">
				<form method="post" action="/wyht/report/ownerReport/"
				...
				<div id="owners-chart" class="m-t-20" style="height: 500px;"></div>
			</div>
		</div>
	</div>
</body>
