...
$(function(){
var houseChart = echarts.init(document.getElementById('house-chart'),'macarons');
houseChart.showLoading();//加载
var disnamedata=[];
	<c:forEach var="item" items="${list }">
		disnamedata.push({name:"${item.dis.dname}",value:"${item.louceng}",disid:"${item.dis.disid}"});
	</c:forEach>
...
//echarts 绑定点击事件
houseChart.on("click", eventClick);
});
//监听事件
function eventClick(param) {
    if (typeof param.seriesIndex != 'undefined') {
		if (param.type == 'click') {
			window.location.href='/wyht/report/housereport/'+param.data.disid;
		}	          
	}
}
